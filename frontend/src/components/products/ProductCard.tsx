...
function ProductCardInner({
    ...
    return (
        <Card className="group flex h-full flex-col hover:shadow-lg transition-all duration-300">
            <div className="relative aspect-square shrink-0 overflow-hidden bg-muted">
                ...
                {isOutOfStock ? (
                    <div className="absolute bottom-2 left-2 right-2 z-20 rounded-lg bg-slate-900/80 px-3 py-2 text-center text-xs font-semibold text-white sm:hidden">
                        Out of Stock
                    </div>
                ) : (
                    <Button
                        size="sm"
                        className="absolute bottom-2 left-2 right-2 z-20 h-9 rounded-lg bg-[#e91e63] text-white shadow-md hover:bg-[#c2185b] sm:hidden"
            ...
            <CardContent className="flex flex-1 flex-col p-3 sm:p-4">
                <Link href={`/products/${product.slug}`}>
                    <h3 className="font-medium line-clamp-2 min-h-[2.5rem] sm:min-h-0 sm:truncate hover:text-primary transition-colors">
                        {product.name}
                    </h3>
                </Link>

                {product.is_discounted_featured && (
                    <div className="mt-2 hidden sm:flex items-center gap-1.5 text-xs font-bold text-rose-500 bg-rose-50 px-2 py-1.5 rounded-md">
                ...
                {product.short_description && (
                    <p className="hidden sm:block text-sm text-muted-foreground truncate mt-1">
                        {product.short_description}
                    </p>
                )}

                <div className="mt-2 sm:mt-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    {product.b2b_price && (
                        <span className="text-[10px] sm:text-xs bg-accent text-accent-foreground px-2 py-1 rounded w-fit">
                ...
                <Button
                    className="mt-auto hidden w-full sm:inline-flex"
